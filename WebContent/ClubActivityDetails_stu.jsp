...
<div class="pic"><img src="images/zx.png"></div>
	 ...
	 <div class="xq_title">
		<h1>${info.title }</h1><%
	 	int flagjoinact = (int)request.getAttribute("flagjoinact");
	 	int flagApplyact = (int)request.getAttribute("flagApplyact");
	 	if(flagjoinact == 1){
	 %>
	 	<p>已加入该活动</p>
	 <%
	 	}
	 	else if(flagApplyact == 1)
	 	{
	 %>	
	 		<p>正在审核</p>
	 <% }
	 	...
	 	{
	 %>
	 <a href="ApplyJoinActServlet?aid=${info.id }">报名参加活动</a>
	 <%
		 }
	 %>
		<p><span>活动日期：${info.time }</span><span>作者：${info.master }</span><span>社团：${info.cname }</span></p>
		
		<p><span><a href="getActPicture?id=${info.id }"target="_blank">	<img src="getActPicture?id=${info.id }"/></a></span></p>
