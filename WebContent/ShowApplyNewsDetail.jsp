	 ...
	 <div class="xq_title">
		<h1>${news.title }	
				<a style="font-size:20px ;	
							...
							cursor: pointer;" href="CheckNewsServlet?id=${news.id }&&con=1">发布</a>
				<a style="font-size:20px ;	
							color:#333;
							background:#D3D3D3;
							padding:5px 15px;
							border-radius: 10px;
							border: 1px solid rgba(0,0,0,0.4) ;
							cursor: pointer;" href="CheckNewsServlet?id=${news.id }&&con=2">驳回</a>
		<p><span>社长：${news.master }</span><span>发布时间：${news.cname }</span></p>
		
		<p><span><a href="getApplyNewsPicture?id=${news.id }"target="_blank">	<img src="getApplyNewsPicture?id=${news.id }"/></a></span></p>
