...
/**
 * Servlet implementation class ApplyActDoServlet
 */
@WebServlet("/ApplyActDoServlet")
public class ApplyActDoServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ApplyActDoServlet() {
        super();
	...
	private boolean savePicture(int cid,String cname,String master,String title,String content,String time ,String filename , InputStream stream,int size) {
		...
		try {
			...
			int sum=0;
			String sql="select count(*) from check_act";
			try{
			...
			stmt=conn.prepareStatement("INSERT INTO check_act VALUES(?,?,?,?,?,?,?,?,?)");
			stmt.setInt(1, sum+1);
			stmt.setInt(2, cid);
